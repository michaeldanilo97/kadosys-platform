 ...
 * caixa aberto por vez por academia - abrirSeNaoHouver() garante isso
...
final class Caixa
{
    ...
    private const SELECT_COLUNAS = 'id, academia_id, usuario_id, status, valor_abertura, valor_fechamento_informado,
    ...
    public static function find(int $id, int $academiaId): ?self
    {
        $stmt = Database::connection()->prepare(
            'SELECT ' . self::SELECT_COLUNAS . ' FROM caixas WHERE id = :id AND academia_id = :academia_id LIMIT 1'
        );
    ...
    public static function aberto(int $academiaId): ?self
    {
        $stmt = Database::connection()->prepare(
            "SELECT " . self::SELECT_COLUNAS . " FROM caixas
             WHERE academia_id = :academia_id AND status = 'aberto'
    ...
    {
        $total = (int) self::contar($academiaId);
        $lastPage = max(1, (int) ceil($total / $perPage));
        $page = max(1, min($page, $lastPage));
        $offset = ($page - 1) * $perPage;

        $stmt = Database::connection()->prepare(
            'SELECT ' . self::SELECT_COLUNAS . " FROM caixas WHERE academia_id = :academia_id
    ...
    public static function abrir(int $academiaId, ?int $usuarioId, float $valorAbertura, ?string $observacoes): int
    {
        $stmt = Database::connection()->prepare(
            "INSERT INTO caixas (academia_id, usuario_id, status, valor_abertura, observacoes_abertura, aberto_em)
             VALUES (:academia_id, :usuario_id, 'aberto', :valor_abertura, :observacoes, NOW())"
        );
        $stmt->execute([
    ...
    public static function fechar(int $id, int $academiaId, float $valorFechamentoInformado, ?string $observacoes): void
    {
        $stmt = Database::connection()->prepare(
            "UPDATE caixas SET status = 'fechado', valor_fechamento_informado = :valor_fechamento,
             ...
             WHERE id = :id AND academia_id = :academia_id AND status = 'aberto'"
        );
        $stmt->execute([
    ...
    private static function contar(int $academiaId): int
    {
        $stmt = Database::connection()->prepare('SELECT COUNT(*) FROM caixas WHERE academia_id = :academia_id');
        $stmt->execute(['academia_id' => $academiaId]);
