 ...
 * como o Academias so tem UMA academia (nao um "central" + N bancos
...
final class WebhookController extends Controller
{
    ...
    private function processarPagamento(MercadoPagoClient $mp, string $paymentId): void
    {
        ...
        $fatura = FaturaAcademia::buscarPorPaymentId($paymentId);

        if ($fatura !== null && $statusInterno === 'autorizada' && $fatura->status === FaturaAcademia::STATUS_PENDENTE) {
            FaturaAcademia::marcarPaga($fatura->id);
            Academia::marcarAtiva($fatura->academiaId);
            Academia::atualizarProximoVencimento($fatura->academiaId, new \DateTimeImmutable('+30 days'));
            // Cobre o caso de reativacao apos o ciclo cancelado ja ter
            // terminado (academia suspensa que gerou uma cobranca nova
            ...
            Academia::cancelarCancelamento($fatura->academiaId);
        }
    ...
    private function processarAssinatura(MercadoPagoClient $mp, string $preapprovalId): void
    {
        ...
        $academia = Academia::buscarPorMpPreapprovalId($preapprovalId);

        if ($academia !== null && $statusInterno === 'autorizada') {
            Academia::marcarAtiva($academia->id);
            Academia::atualizarProximoVencimento($academia->id, new \DateTimeImmutable('+30 days'));
