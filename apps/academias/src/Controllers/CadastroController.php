...
use Academias\Models\Unidade;
use Academias\Models\User;

/**
 * Cadastro publico autoatendido: academia + administrador + plano,
 ...
 * academia (e o usuario admin dela) ja sao criados na hora, com
...
final class CadastroController extends Controller
{
    ...
    public function form(): void
    {
        ...
        echo $this->view('cadastro.form', [
            'pageTitle' => 'Criar conta - KADOSYS Academias',
            'csrf' => Csrf::field(),
    ...
    public function enviar(): void
    {
        ...
        if ($metodoPagamento === 'trial' && $documento !== '' && Academia::documentoJaUsouTrial($documento)) {
        ...
        Unidade::criarPrincipal($academiaId, $nome);
        User::create($academiaId, $adminNome, $adminEmail, $senha);
        ...
        $referenciaExterna = 'kadosys-academias-' . $academiaId . '-' . bin2hex(random_bytes(4));

        if ($metodoPagamento === 'pix') {
            $this->criarCobrancaPix($mp, $academiaId, $plano, $adminEmail, $referenciaExterna, $old);
        }

        try {
            $resposta = $mp->criarAssinatura([
                // O Mercado Pago limita "reason" a 40 caracteres.
                'reason' => 'KADOSYS Academias - Plano ' . Plano::label($plano),
        ...
        Academia::atualizarMpPreapprovalId($academiaId, (string) $resposta['body']['id']);
    ...
    private function criarCobrancaPix(
        ...
        try {
            $resposta = $mp->criarPagamentoPix([
                'description' => 'KADOSYS Academias - Plano ' . Plano::label($plano),
                'amount' => Plano::valorMensal($plano),
                'payerEmail' => $adminEmail,
                'externalReference' => $referenciaExterna,
                'expiraEm' => $vencimento,
    ...
    {
        $fatura = FaturaAcademia::find((int) $id);
        ...
        echo $this->view('cadastro.pix', [
            'pageTitle' => 'Pagar com Pix - KADOSYS Academias',
            'fatura' => $fatura,
        ], 'site');
    }
     ...
     * academia como ativa) e feita pelo webhook do Mercado Pago de
    ...
    public function retorno(): void
    {
        echo $this->view('cadastro.retorno', [
            'pageTitle' => 'Cadastro em processamento - KADOSYS Academias',
        ], 'site');
    }
    ...
    private function validar(
        ...
        } elseif (!Academia::slugDisponivel($slug)) {
        ...
        if ($documentoTipo === 'cnpj' && ($razaoSocial === '' || mb_strlen($razaoSocial) < 3)) {
            $errors[] = 'Informe a razão social da academia.';
        }
    ...
    /**
     * Normaliza um texto livre (nome da academia ou identificador
