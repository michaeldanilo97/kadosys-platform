 ...
 * @var array<int, FaturaAcademia> $faturasPix
...
$statusFaturaLabel = [
    FaturaAcademia::STATUS_PENDENTE => ['Pendente', 'warn'],
    FaturaAcademia::STATUS_PAGA => ['Paga', 'ok'],
    FaturaAcademia::STATUS_EXPIRADA => ['Expirada', 'dim'],
    FaturaAcademia::STATUS_CANCELADA => ['Cancelada', 'dim'],
];
...
<main class="dashboard-main">
    ...
    <?php if ($academia->metodoPagamento === 'cartao'): ?>
                    ...
                    <tbody>
                        <?php foreach ($faturasPix as $fatura): ?>
                            ...
                            <tr>
                                ...
                                <td class="text-dim"><?= htmlspecialchars(\Academias\Models\Plano::label($fatura->plano), ENT_QUOTES, 'UTF-8') ?></td>
                                <td>R$ <?= number_format($fatura->valor, 2, ',', '.') ?></td>
                                <td><span class="status-badge <?= $badgeFatura ?>"><?= $labelFatura ?></span></td>
                                <td>
                                    <?php if ($fatura->status === FaturaAcademia::STATUS_PAGA && $fatura->pagoEm !== null): ?>
                                        <span class="text-dim">Pago em <?= (new DateTimeImmutable($fatura->pagoEm))->format('d/m/Y') ?></span>
                                    <?php elseif ($fatura->status === FaturaAcademia::STATUS_PENDENTE): ?>
