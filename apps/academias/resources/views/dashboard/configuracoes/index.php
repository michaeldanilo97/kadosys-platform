...
<main class="dashboard-main">
    ...
    <div class="glass-card dash-panel">
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:1rem; margin-bottom:1.25rem; align-items:stretch;">
            <?php foreach (Plano::LABELS as $planoValor => $planoNome): ?>
                <?php $ehAtual = $academia->plano === $planoValor; ?>
        ...
        <?php else: ?>
            <div style="margin: 1.25rem 0 0; padding-top: 1.25rem; border-top: 1px solid var(--glass-border);">
                <div class="auth-alert error" style="margin-bottom: 0.75rem;">
                    Assinatura cancelada<?= $academia->proximoVencimento !== null ? ' - acesso liberado até ' . (new DateTimeImmutable($academia->proximoVencimento))->format('d/m/Y') . '.' : '.' ?>
                </div>
    ...
    <div class="glass-card dash-panel">
        ...
        <form method="POST" action="<?= $basePath ?>/dashboard/configuracoes/pix" class="crud-form-grid">
            <div class="form-field">
                ...
                <input type="text" id="pix_chave" name="pix_chave" value="<?= htmlspecialchars($academia->pixChave ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="CPF, CNPJ, e-mail, telefone ou chave aleatória">
            </div>
            <div class="form-field">
                <label for="pix_nome_beneficiario">Nome do beneficiário</label>
                <input type="text" id="pix_nome_beneficiario" name="pix_nome_beneficiario" value="<?= htmlspecialchars($academia->pixNomeBeneficiario ?? '', ENT_QUOTES, 'UTF-8') ?>" maxlength="25" placeholder="Como aparece no app do banco">
            </div>
            <div class="form-field">
                <label for="pix_cidade">Cidade</label>
                <input type="text" id="pix_cidade" name="pix_cidade" value="<?= htmlspecialchars($academia->pixCidade ?? '', ENT_QUOTES, 'UTF-8') ?>" maxlength="15" placeholder="Sua cidade">
            </div>
            <?= Csrf::field() ?>
        ...
        <form method="POST" action="<?= $basePath ?>/dashboard/configuracoes/perfil" enctype="multipart/form-data">
            ...
            <div class="crud-form-grid">
                <div class="form-field">
                    ...
                    <input type="text" id="nome" name="nome" value="<?= htmlspecialchars($academia->nome, ENT_QUOTES, 'UTF-8') ?>" required>
                </div>
                <div class="form-field">
                    <label for="telefone">Telefone</label>
                    <input type="text" id="telefone" name="telefone" value="<?= htmlspecialchars($academia->telefone ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="(11) 90000-0000">
                </div>
                <div class="form-field">
                    <label for="cor_primaria">Cor de destaque</label>
                    <input type="color" id="cor_primaria" name="cor_primaria" value="<?= htmlspecialchars($academia->corPrimaria ?? '#3B82F6', ENT_QUOTES, 'UTF-8') ?>" style="height: 2.7rem; padding: 0.25rem;">
                ...
                <div class="form-field">
                    ...
                    <?php if ($academia->logoPath): ?>
                        <img src="<?= $basePath ?>/<?= htmlspecialchars($academia->logoPath, ENT_QUOTES, 'UTF-8') ?>" alt="Logo atual" class="foto-preview">
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="glass-card dash-panel">
        <div class="dash-panel-head">
            <h2>Equipe</h2>
            <p>Quem tem acesso ao painel da academia.</p>
        </div>
