 ...
 * como o Food so tem UM restaurante (nao um "central" + N bancos
...
final class WebhookController extends Controller
{
    ...
    private function processarPagamento(MercadoPagoClient $mp, string $paymentId): void
    {
        ...
        $fatura = FaturaRestaurante::buscarPorPaymentId($paymentId);

        if ($fatura !== null && $statusInterno === 'autorizada' && $fatura->status === FaturaRestaurante::STATUS_PENDENTE) {
            FaturaRestaurante::marcarPaga($fatura->id);
            Restaurante::marcarAtiva($fatura->restauranteId);
            Restaurante::atualizarProximoVencimento($fatura->restauranteId, new \DateTimeImmutable('+30 days'));
            // Cobre o caso de reativacao apos o ciclo cancelado ja ter
            // terminado (restaurante suspenso que gerou uma cobranca nova
            ...
            Restaurante::cancelarCancelamento($fatura->restauranteId);
        }
    ...
    private function processarAssinatura(MercadoPagoClient $mp, string $preapprovalId): void
    {
        ...
        $restaurante = Restaurante::buscarPorMpPreapprovalId($preapprovalId);

        if ($restaurante !== null && $statusInterno === 'autorizada') {
            Restaurante::marcarAtiva($restaurante->id);
            Restaurante::atualizarProximoVencimento($restaurante->id, new \DateTimeImmutable('+30 days'));
            Restaurante::cancelarCancelamento($restaurante->id);
        }
