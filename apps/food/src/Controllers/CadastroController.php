...
/**
 * Cadastro publico autoatendido: restaurante + administrador + plano,
 ...
 * como nao ha banco/subdominio pra provisionar por cliente, o
 * restaurante (e o usuario admin dele) ja sao criados na hora, com
...
final class CadastroController extends Controller
{
    ...
    public function form(): void
    {
        ...
        echo $this->view('cadastro.form', [
            'pageTitle' => 'Criar conta - KADOSYS Food',
            'csrf' => Csrf::field(),
    ...
    public function enviar(): void
    {
        ...
        if ($metodoPagamento === 'trial' && $documento !== '' && Restaurante::documentoJaUsouTrial($documento)) {
        ...
        $restauranteId = Restaurante::criar(
            $nome,
            $slug,
            $documentoTipo,
            $documento,
        ...
        $referenciaExterna = 'kadosys-food-' . $restauranteId . '-' . bin2hex(random_bytes(4));

        if ($metodoPagamento === 'pix') {
            $this->criarCobrancaPix($mp, $restauranteId, $plano, $adminEmail, $referenciaExterna, $old);
        }

        try {
            $resposta = $mp->criarAssinatura([
                // O Mercado Pago limita "reason" a 40 caracteres.
                'reason' => 'KADOSYS Food - Plano ' . Plano::label($plano),
        ...
        Restaurante::atualizarMpPreapprovalId($restauranteId, (string) $resposta['body']['id']);
    ...
    private function criarCobrancaPix(
        ...
        try {
            $resposta = $mp->criarPagamentoPix([
                'description' => 'KADOSYS Food - Plano ' . Plano::label($plano),
                'amount' => Plano::valorMensal($plano),
                'payerEmail' => $adminEmail,
                'externalReference' => $referenciaExterna,
                'expiraEm' => $vencimento,
    ...
    {
        $fatura = FaturaRestaurante::find((int) $id);
        ...
        echo $this->view('cadastro.pix', [
            'pageTitle' => 'Pagar com Pix - KADOSYS Food',
            'fatura' => $fatura,
        ], 'site');
    }
    ...
    /**
     * Pagina de retorno do Checkout Pro. A confirmacao em si (marcar o
     * restaurante como ativo) e feita pelo webhook do Mercado Pago de
    ...
    public function retorno(): void
    {
        echo $this->view('cadastro.retorno', [
            'pageTitle' => 'Cadastro em processamento - KADOSYS Food',
        ], 'site');
    }
    ...
    private function validar(
        ...
        } elseif (!Restaurante::slugDisponivel($slug)) {
        ...
        if ($documentoTipo === 'cnpj' && ($razaoSocial === '' || mb_strlen($razaoSocial) < 3)) {
            $errors[] = 'Informe a razão social do restaurante.';
        }
    ...
    /**
     * Normaliza um texto livre (nome do restaurante ou identificador
