 ...
 * @var array<int, FaturaBarbearia> $faturasPix
...
$statusFaturaLabel = [
    FaturaBarbearia::STATUS_PENDENTE => ['Pendente', 'warn'],
    FaturaBarbearia::STATUS_PAGA => ['Paga', 'ok'],
    FaturaBarbearia::STATUS_EXPIRADA => ['Expirada', 'dim'],
    FaturaBarbearia::STATUS_CANCELADA => ['Cancelada', 'dim'],
];
...
<main class="dashboard-main">
    ...
    <?php if ($barbearia->metodoPagamento === 'cartao'): ?>
                    ...
                    <tbody>
                        <?php foreach ($faturasPix as $fatura): ?>
                            ...
                            <tr>
                                ...
                                <td class="text-dim"><?= htmlspecialchars(\Barbearias\Models\Plano::label($fatura->plano), ENT_QUOTES, 'UTF-8') ?></td>
                                <td>R$ <?= number_format($fatura->valor, 2, ',', '.') ?></td>
                                <td><span class="status-badge <?= $badgeFatura ?>"><?= $labelFatura ?></span></td>
                                <td>
                                    <?php if ($fatura->status === FaturaBarbearia::STATUS_PAGA && $fatura->pagoEm !== null): ?>
                                        <span class="text-dim">Pago em <?= (new DateTimeImmutable($fatura->pagoEm))->format('d/m/Y') ?></span>
                                    <?php elseif ($fatura->status === FaturaBarbearia::STATUS_PENDENTE): ?>
