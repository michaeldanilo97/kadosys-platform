...
/*
|--------------------------------------------------------------------------
| Cron - Suspende barbearias com assinatura cancelada (KADOSYS Barbearias)
|--------------------------------------------------------------------------
|
| Roda uma vez por dia (configurar no "Cron Jobs" do cPanel, ex.:
| php /home/kadosys1/apps/barbearias/cron/suspender_assinaturas_canceladas.php).
|
...
if (realpath($argv[0] ?? '') === __FILE__) {
    $barbearias = Barbearia::canceladasComCicloEncerrado();

    log_cron_suspensao(sprintf('%d barbearia(s) com assinatura cancelada e ciclo pago encerrado.', count($barbearias)));

    foreach ($barbearias as $barbearia) {
        Barbearia::marcarSuspensa($barbearia->id);
        log_cron_suspensao("Barbearia {$barbearia->slug}: acesso suspenso (assinatura cancelada em {$barbearia->canceladoEm}).");
    }
