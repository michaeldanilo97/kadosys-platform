...
<div class="auth-form-card">
    ...
    <h1><?= $confirmada ? 'Obrigado pela doação!' : 'Pague com Pix' ?></h1>

    <?php if ($confirmada): ?>
        <div class="auth-alert" style="background: rgba(52,211,153,0.12); border-color: rgba(52,211,153,0.35); color: var(--success);">
            <i class="bi bi-check-circle"></i> Recebemos sua confirmação. Que Deus abençoe sua generosidade!
        </div>
    <?php else: ?>
        <p class="subtitle">Escaneie o QR code no app do seu banco, ou use o código "copia e cola". Depois de pagar, confirme aqui embaixo.</p>
    <?php endif; ?>
    ...
    <?php if (!$confirmada): ?>
        ...
        <form method="POST" action="<?= $basePath ?>/doar/<?= $doacao->id ?>/confirmar">
            <?= $csrf ?>
            <button type="submit" class="btn-k btn-k-grad">Já fiz o Pix <i class="bi bi-check2"></i></button>
        </form>

        <p class="auth-field-hint" style="margin-top: 0.8rem; text-align: center;">
            O Pix cai direto na conta da igreja - o sistema não recebe confirmação automática do banco, por isso pedimos que você confirme aqui.
        </p>
    <?php endif; ?>

    <a href="<?= $basePath ?>/doar" class="auth-back-link">
        <i class="bi bi-arrow-left"></i> Fazer outra doação
    </a>
</div>
