 ...
 * partir da chave Pix da propria igreja - sem nenhuma chamada a gateway
 * de pagamento. O dinheiro cai direto na conta da igreja; o sistema so
 * monta a string que vira QR code (renderizado no navegador, ver
 * assets/js/vendor/qrcode-generator.js) e o texto "copia e cola".
 *
 * Trade-off assumido (ver conversa que originou esta feature): sem
 * gateway, nao ha confirmacao automatica de pagamento - o doador
 * confirma manualmente que fez o Pix (ver Igrejas\Models\FinanceiroDoacao).
 */
final class PixEstatico
{
    ...
    /**
     * @param string $chave Chave Pix da igreja (CPF, CNPJ, e-mail, telefone ou chave aleatoria)
    ...
    private static function normalizarTexto(string $texto, int $tamanhoMaximo): string
    {
        ...
        return substr($limpo !== '' ? $limpo : 'IGREJA', 0, $tamanhoMaximo);
    }
    ...
    private static function normalizarTxid(string $txid): string
    {
        ...
        return substr($limpo !== '' ? $limpo : 'DOACAO', 0, 25);
    }
